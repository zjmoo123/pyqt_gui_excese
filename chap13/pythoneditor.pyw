...
from chap13 import qrc_resources
import os
import sys
...
class TextEdit(QTextEdit):
    ...
    def event(self, QEvent):
        if QEvent.type() == QEvent.KeyPress and QEvent.key() == Qt.Key_Tab:
            ...
            cursor.insertText("    ")
            return True
        return QTextEdit.event(self, QEvent)
...
class PythonHighlighter(QSyntaxHighlighter):
    ...
    def __init__(self, parent=None):
        ...
        KEYWORDS = ["and", "as", "assert", "break", "class",
                    ...
                    "if", "import", "in", "is", "lamba", "not", "or", "pass",
        ...
        BUILTINS = ["abs", "all", "any", "basestring", "bool",
                    ...
                    "vars", "zip"]
        CONSTANTS = ["False", "True", "None", "NotImplement",
                     "Ellipsis"]
        ...
        PythonHighlighter.Rules.append((QRegExp("|".join([r"\b%s\b" % constant for constant in CONSTANTS])), "builtin"))
    ...
    def initializeFormats():
        ...
        for name, color in (("normal", Qt.black), ("keyword", Qt.darkBlue), ("builtin", Qt.darkRed),
                            ...
                            ("string", Qt.darkYellow), ("number", Qt.darkMagenta), ("error", Qt.darkRed),
                            ("pyqt", Qt.darkCyan)):
    ...
    def highlightBlock(self, text):
        ...
        if (prevState == ERROR and not (text.startswith(sys.psl) or text.startswith("#"))):
            self.setCurrentBlockState(ERROR)
            self.setFormat(0, textLength, PythonHighlighter.Formats["error"])
            return
        ...
        for i, state in ((self.tripleSingleRe.indexIn(text), TRIPLESINGLE),
            ...
            if self.previousBlockState() == state:
                ...
                self.setCurrentBlockState(state)
                self.setFormat(i, text.length(), PythonHighlighter.Formats["string"])
