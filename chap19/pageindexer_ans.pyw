...
from chap19 import walker_ans as walker
...
class Form(QDialog):
    ...
    def setPath(self):
        ...
        path = QFileDialog.getExistingDirectory(self, "Choose a path to Index", self.path)
        if not path:
    ...
    def find(self):
        ...
        try:
            ...
            self.statusLabel.setText("{0} indexed files contain the word '{1}'".format(
                len(files), word))
        finally:
            self.mutex.unlock()
