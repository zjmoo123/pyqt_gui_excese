...
class TextItem(QGraphicsTextItem):
    def __init__(self, text, position, scene, font=QFont("Times", PointSize), matrix=QTransform()):
...
class BoxItem(QGraphicsItem):
    def __init__(self, position, scene, style=Qt.SolidLine, rect=None, matrix=QTransform()):
    ...
    def paint(self, QPainter, QStyleOptionGraphicsItem, widget):
        pen = QPen(self.style)
        pen.setColor(Qt.black)
        pen.setWidth(1)
        if QStyleOptionGraphicsItem.state & QStyle.State_Selected:
...
class MainForm(QDialog):
    ...
    def addPixmap(self):
        ...
        fname, filetype = QFileDialog.getOpenFileName(self, "Page Designer - Add Pixmap", path,
