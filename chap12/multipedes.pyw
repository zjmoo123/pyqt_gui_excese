...
class MainForm(QDialog):
    ...
    def pauseOrResume(self):
        ...
        self.pauseButton.setText("Ps&use" if self.running else "Res&ume")
...
class Head(QGraphicsItem):
    ...
    def paint(self, painter, option, widget=None):
        ...
        if option.levelOfDetailFromTransform(self.transform()) > 0.5:
            painter.setBrush(QBrush(Qt.yellow))
            painter.drawEllipse(-12, -19, 8, 8)
            painter.drawEllipse(-12, 11, 8, 8)
            if option.levelOfDetailFromTransform(self.transform()) > 0.8:
...
class Segment(QGraphicsItem):
    ...
    def paint(self, QPainter, QStyleOptionGraphicsItem, widget=None):
        ...
        QPainter.setBrush(QBrush(self.color))
        if QStyleOptionGraphicsItem.levelOfDetailFromTransform(self.transform()) < 0.9:
