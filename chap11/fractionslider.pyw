...
class FractionSlider(QWidget):
    ...
    def keyPressEvent(self, QKeyEvent):
        ...
        elif QKeyEvent.key() in (Qt.Key_Up, Qt.Key_Right):
            change = 1
        elif QKeyEvent.key() == Qt.Key_PageUp:
            change = (self.__denominator // 10) + 1
        elif QKeyEvent.key() in (Qt.Key_Down, Qt.Key_Left):
            change = -1
    ...
    def paintEvent(self, QPaintEvent=None):
        ...
        triangle = [QPointF(value * span, y), QPointF((value * span) + (2 * FractionSlider.XMARGIN), y),
