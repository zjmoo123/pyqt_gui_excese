...
class ReferenceDataDlg(QDialog):
    def __init__(self, table, title, parent=None):
        ...
        self.setWindowTitle("Asset Manager - Edit {0} Reference Data".format(title))
...
class LogDelegate(QSqlRelationalDelegate):
    ...
    def createEditor(self, parent, option, index):
        if (index.column() == ACTIONID and index.model().data(index, Qt.DisplayRole) == ACQUIRED):
            return
        if index.column() == DATE:
