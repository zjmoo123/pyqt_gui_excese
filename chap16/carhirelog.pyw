...
from chap16 import genericdelegates
...
class CarHireModel(QAbstractTableModel):
    ...
    def __init__(self, parent=None):
        ...
        import random
        import string
        surname_data = gzip.open(os.path.join(os.path.dirname(__file__), "surnames.txt.gz")).read()
        ...
        for i in range(250):
            ...
            if random.random() >= 0.2:
                ...
                returned = QDate()
                mileageback = 0
            log = CarHireLog(license, customer, hired, mileageout, returned, mileageback, notes)
    ...
    def setData(self, index, value, role=Qt.EditRole):
        if (index.isValid() and role == Qt.EditRole and index.column() not in (LICENSE, MILEAGE, DAYS)):
            log = self.logs[index.row()]
            column = index.column()
            if column == CUSTOMER:
                log.customer = value
...
class HireDateColumnDelegate(genericdelegates.DateColumnDelegate):
    def createEditor(self, parent, option, index):
        ...
        return genericdelegates.DateColumnDelegate.createEditor(self, parent, option, index)
...
class MileageOutColumnDelegate(genericdelegates.IntegerColumnDelegate):
    def createEditor(self, parent, option, index):
        ...
        return genericdelegates.IntegerColumnDelegate.createEditor(self, parent, option, index)
...
class MainForm(QMainWindow):
    def __init__(self, parent=None):
        ...
        delegate.insertColumnDelegate(CUSTOMER, genericdelegates.PlainTextColumnDelegate())
        ...
        delegate.insertColumnDelegate(MILEAGEBACK, MileageBackColumnDelegate(0, 1000000))
